...
def getTitles(users):
    return[user['title'] for user in users ]
    
def printDesc(movies):
    return [movie['description'] for movie in movies]
    
def noVowelDesc(movies):
    return []

def getDirPro(movies):
    return [f"{movie['director']} {movie['producer']}" for movie in movies]

    
def getScore(movies):
    return [f"The movie {movie['title']} got a Rotten Tomato score of {movie['score']}." for movie in movies]
    
    
def movieInfo(movies):
    return [f"{movie.title} was directed by {movie['director']}, and released on{movie['date']}" for movie in movies]
    
def bestMovies(movies):
    return [movie for movie in movies if movie['director'] == 'Hayao Miyazaki']
    
def shortenedYears(movies):
    return [f"{movie['title']}: '{movie['release_date'].[:2]}" for movie in movies]